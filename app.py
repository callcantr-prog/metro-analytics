...
def init_db():
    ...
    """)
    existing_columns = {
        row[1] for row in con.execute("PRAGMA table_info(monthly_plans)")
    }
    for column, definition in (
        ...
        if column not in existing_columns:
            con.execute(f"ALTER TABLE monthly_plans ADD COLUMN {column} {definition}")
...
def tg_send(chat_id, text, keyboard=None):
    ...
    # Главное меню больше не добавляется под сообщениями.
    # Меню бота открывается через стандартную кнопку Telegram
    # возле поля ввода (Menu Button).
    if keyboard is None:
        keyboard = None
...
def set_monthly_plan(plan_key, month_key, amount, chat_id=None, user_id=None, user_name=None):
    con = sqlite3.connect(DB)
    con.execute(
        """INSERT INTO monthly_plans(
               plan_key, month, amount, updated_at, chat_id, user_id, user_name
           )
           VALUES(?,?,?,?,?,?,?)
...
def get_active_monthly_plans(month_key):
    con = sqlite3.connect(DB)
    rows = con.execute(
        """SELECT plan_key, month, amount, chat_id, user_id, user_name
           FROM monthly_plans
           WHERE month=? AND chat_id IS NOT NULL""",
        (month_key,)
    ).fetchall()
    con.close()
    return rows
...
def send_plan_daily_summary():
    """Персональный итог по сохранённому месячному плану в 22:00 в последний день месяца."""
    today = datetime.now(TZ).date()
    month_key = today.strftime("%Y-%m")

    for plan_key, _, amount, chat_id, user_id, user_name in get_active_monthly_plans(month_key):
        if not chat_id:
            continue
        try:
            month_start = today.replace(day=1)
            fact = float(get_period(month_start, today).get("revenue", 0) or 0)
            plan = float(amount or 0)
            percent = (fact / plan * 100) if plan else 0
            name = user_name or "Коллега"

            tg_send(
                chat_id,
                f"🎯 {name}, итоги на {today.strftime('%d.%m.%Y')}\n\n"
                f"Ваш план на месяц: {money(plan)}\n"
                f"Выполнено: {money(fact)}\n"
            ...
            print("Plan daily summary error:", repr(exc))
...
def instruction():
    return (
        "ℹ️ Что умеет МЕТРО — Аналитик\n\n"
        "📊 Сегодня — выручка, визиты и средний чек.\n"
        "👨‍💼 Мастера — показатели и сравнение мастеров.\n"
        "📈 Динамика — сравнение периодов и изменения показателей.\n"
        "🧠 Анализ — поиск причин изменений и точек роста.\n"
        "👥 Клиенты — база, уходящие и потерянные клиенты.\n"
        "💈 Услуги — продажи и выручка по услугам.\n"
        "📣 Маркетинг — анализ результатов продвижения.\n"
        "🎯 План / факт — месячный план, факт, выполнение и остаток.\n"
        "🤖 Спроси аналитика — ИИ анализирует данные МЕТРО и помогает принимать решения.\n\n"
        "🔎 Находит проблемы — показывает, где проседают показатели.\n"
        "📈 Ищет точки роста — помогает понять, за счёт чего увеличить выручку.\n"
        "💡 Даёт рекомендации — предлагает конкретные действия на основе аналитики.\n\n"
        "Данные формируются на основе YCLIENTS."
    )
...
def route_message(text, plan_key="default"):
    ...
    if text in ("/план", "план / факт", "🎯 план / факт"):
        return report_plan(plan_key), plan_keyboard()
    ...
    if callback:
        ...
        try:
            ...
            if callback_chat_id and data == "end_analyst":
                ...
                tg_send(
                    callback_chat_id,
                    "🤖 Диалог с аналитиком завершен, надеюсь был вам полезен."
                )
                return jsonify({"ok": True})
            ...
            if callback_chat_id and data.startswith("menu:"):
                ...
                if menu_command == "динамика":
                    ...
                    if menu_command == "план":
                        ...
                        callback_user_id = callback_user.get("id")
                        plan_key = (
                            f"{callback_chat_id}:{callback_user_id}"
                            if callback_user_id is not None
                            else "default"
                        )
                        answer, keyboard = report_plan(plan_key), plan_keyboard()
                    else:
                        callback_user = callback.get("from") or {}
                        callback_user_id = callback_user.get("id")
                        route_plan_key = (
                        ...
                        answer, keyboard = route_message(
                            menu_command, route_plan_key
                        )
        ...
        try:
            ...
            if cid in plan_states:
                ...
                try:
                    ...
                    set_monthly_plan(
                        cid,
                        datetime.now(TZ).date().strftime("%Y-%m"),
                        amount,
                        chat_id=chat_id,
                        user_id=user_id,
                        user_name=user_name
            ...
            answer, keyboard = route_message(text, cid)
...
def scheduler():
    ...
    while True:
        try:
            ...
            # Итог по плану отправляем в 22:00 только в последний
            # календарный день текущего месяца (28/29/30/31 — по календарю).
            last_day = calendar.monthrange(now.year, now.month)[1]
            if now.day == last_day and now.hour == 22 and now.minute < 5:
                if plan_summary_sent != day_key:
                    plan_summary_sent = day_key
                    send_plan_daily_summary()
...
def configure_telegram():
    ...
    try:
        ...
        # Для групп отдельно задаём тот же список команд.
        # Это убирает старую команду «Финансы» из квадратика меню
        # и возвращает «Клиенты», если для группы ранее был сохранён
        # отдельный набор команд. Остальную логику бота не меняем.
        group_commands_response = requests.post(
            f"{TG}/setMyCommands",
            json={
                "commands": commands,
        ...
        print(
            ...
            group_commands_response.status_code,
            group_commands_response.text[:500]
        )

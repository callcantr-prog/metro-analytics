...
def ask_groq(question, history=None):
    ...
    try:
        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "Если данных недостаточно, честно скажи об этом и дай полезный вывод из имеющихся данных. "
                    "Учитывай предыдущие сообщения диалога и отвечай на уточнения по смыслу. "
                    "Отвечай по-русски, понятно и конкретно, как опытный бизнес-аналитик. "
                    "Если вопрос требует сравнения, показывай конкретные цифры и процент изменения. "
                    "Формат для Telegram: короткие абзацы, понятные списки, без таблиц. "
                    "НИКОГДА не используй LaTeX и математические команды: "
                    "не пиши \\frac, \\text, \\[ \\], \\( \\), $...$. "
                    "Формулы пиши обычным текстом, например: 359 400 ₽ / 429 = 837,5 ₽. "
                    "Не начинай ответ с повторения всей аналитики. Сначала дай прямой ответ на вопрос, "
                    "затем кратко объясни вывод и предложи конкретные действия."
                )
            }
        ]

        # В историю передаём только реальные реплики диалога.
        ...
        ai_log(f"Отправляю запрос в Groq: model={payload.get('model')}, question={question[:200]!r}")
        response = requests.post(
        ...
        ai_log(f"Ответ Groq получен: HTTP {response.status_code}")
        if response.status_code != 200:
            ai_log(f"Groq HTTP error {response.status_code}: {response.text[:2000]}")
            # Если модель недоступна, пробуем стабильную модель Groq.
        ...
        if response.status_code != 200:
            ai_log(f"Groq final HTTP error {response.status_code}: {response.text[:2000]}")
            return (
                "⚠️ ИИ временно не смог ответить. "
                "Проверь подключение к Groq и GROQ_API_KEY."
            )
        ...
        if not answer:
            ai_log(f"Groq empty response: {data}")
            return "⚠️ ИИ вернул пустой ответ. Попробуйте задать вопрос ещё раз."

        return clean_analyst_answer(answer)

    except requests.Timeout:
        ai_log("Groq timeout")
        return "⚠️ ИИ не успел ответить. Попробуйте ещё раз."
    except requests.RequestException as exc:
        ai_log(f"Groq request error: {exc!r}")
        return "⚠️ Нет соединения с ИИ. Попробуйте ещё раз."
    except Exception as exc:
        ai_log(f"Groq error: {exc!r}")
        return "⚠️ Не удалось получить ответ аналитика. Попробуйте ещё раз."

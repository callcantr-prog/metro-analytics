...
def report_dynamics(selected_start=None, selected_end=None):
    ...
    if selected_start and selected_end:
        ...
        lines.extend([
            "",
            f"📅 Выбранный период: {selected_start.strftime('%d.%m.%Y')} — {selected_end.strftime('%d.%m.%Y')}",
            f"Сравнение: {previous_start.strftime('%d.%m.%Y')} — {previous_end.strftime('%d.%m.%Y')} | выбранный",
...
def route_message(text):
    ...
    if text in ("/analitik", "спроси аналитика", "🤖 спроси аналитика"):
        return (
            ...
            "Напишите ваш вопрос, и я отвечу на основе данных МЕТРО из YCLIENTS.",
            None
        )
...
def telegram_webhook():
    ...
    if callback:
        ...
        try:
            ...
            if callback_chat_id and data == "ask_analyst":
                ...
                analytics_questions.add(f"{callback_chat_id}:{callback_user_id}")
                tg_send(
                    callback_chat_id,
                    "🤖 Режим аналитика включён.\n\nНапишите ваш вопрос, и я отвечу на основе данных МЕТРО из YCLIENTS.",
                    None
            ...
            if callback_chat_id and data.startswith("menu:"):
                ...
                if menu_command == "динамика":
                    ...
                    answer, _ = route_message(menu_command)
                    tg_send(callback_chat_id, answer, None)
                return jsonify({"ok": True})
        except Exception as exc:
    ...
    if chat_id and text:
        try:
            ...
            if cid in analytics_questions:
                analytics_questions.discard(cid)
                tg_send(chat_id, ask_groq(text), None)
                return jsonify({"ok": True})

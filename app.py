...
def ask_groq(question, history=None):
    """Ответ аналитика с контекстом YCLIENTS и историей текущего диалога."""
    if not GROQ_API_KEY:
    ...
    try:
        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "Отвечай непосредственно на каждый заданный вопрос, а не выдавай "
                    "просто общий отчёт. Используй актуальные данные YCLIENTS из контекста. "
                    "Можно делать расчёты и выводы только из переданных данных. "
                    "Не выдумывай цифры, имена мастеров или причины, которых нет в данных. "
                    "Если данных недостаточно для точного вывода — скажи, каких данных не хватает, "
                    "но всё равно дай полезный вывод из того, что есть. "
                    "Если пользователь задаёт уточняющий вопрос, учитывай предыдущие сообщения "
                    "диалога и не начинай разговор заново. "
                    "Отвечай по-русски, понятно и конкретно. Если вопрос требует сравнения, "
                    "покажи конкретные цифры и процент изменения, когда его можно посчитать. "
                    "Формат ответа предназначен для Telegram и должен легко читаться с телефона. "
                    "НИКОГДА не используй LaTeX: не пиши \\frac, \\text, \\[ \\], \\( \\), "
                    "$...$ или другие математические команды. Формулы пиши только обычным текстом, "
                    "например: 359 400 ₽ / 429 = 837,5 ₽. "
                    "Не используй таблицы и сложную разметку. "
                    "Делай короткие абзацы и списки с понятными заголовками. "
                    "Сначала дай прямой ответ на вопрос, затем кратко объясни почему и что делать. "
                    "Не повторяй весь контекст аналитики, если пользователь этого не просил."
                )
            }
        ]
        ...
        return clean_analyst_answer(response.choices[0].message.content)
    except Exception as exc:

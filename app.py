...
def not_found(error):
    # Helpful diagnostics instead of a plain "Not Found" while configuring Render.
    return jsonify({
        "ok": False,
        "error": "not_found",
        "path": request.path,
...
def yclients_webhook():
    # GET is intentionally supported for diagnostics. YCLIENTS sends POST JSON.
    ...
    # Always acknowledge quickly. YCLIENTS expects a successful HTTP response.
    return jsonify({
        "ok": True,
        "telegram_sent": sent,
        "received": {
...
@app.get("/register")
def register():
    # Safe registration/landing endpoint for the YCLIENTS application.
...
# Render starts this application with Gunicorn (`gunicorn app:app`), so this
# must run on import as well; otherwise Telegram's webhook would never be set.

    ...
    if callback:
        ...
        try:
            ...
            if callback_chat_id and data == "main_menu":
                # Не отправляем отдельное сообщение меню. Показываем меню
                # прямо под сообщением, на котором нажата кнопка «Главное меню».
                callback_message_id = callback_message.get("message_id")
                if TG and callback_message_id:
                    requests.post(
                        f"{TG}/editMessageReplyMarkup",
                        json={
            ...
            if callback_chat_id and data == "end_analyst":
                ...
                # Не отправляем отдельное сообщение с главным меню.
                # Меню появляется на месте текущего сообщения после нажатия
                # кнопки «🛑 Закончить диалог».
                callback_message_id = callback_message.get("message_id")
                if TG and callback_message_id:
                    requests.post(
                        ...
                        json={
                            "chat_id": callback_chat_id,
                            "message_id": callback_message_id,
                            "text": "🤖 Диалог с аналитиком завершён.\n\nВыберите нужный раздел:",
                            "reply_markup": main_menu(),
                        },
                        timeout=10,
                    )

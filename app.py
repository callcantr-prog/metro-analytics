...
def menu_button():
    return {
        "inline_keyboard": [
            [{"text": "☰ Главное меню", "callback_data": "menu:open"}]
        ]
    }
...
def report_today():
    ...
    return (
        ...
        "📈 К вчера\n"
        f"Выручка: {pct_change(current['revenue'], previous['revenue'])}\n"
        f"Клиенты: {pct_change(current['clients_count'], previous['clients_count'])}\n"
        f"Средний чек: {pct_change(current['avg'], previous['avg'])}"
    )
...
def report_master_question(question=""):
    ...
    def one(x):
        ...
        if x["noshow"]:
            reasons.append(f"неявок: {x['noshow']}")
        if not reasons:
...
def report_clients():
    ...
    # Уходящие и потерянные считаются за последние 3 месяца.
    leaving = []
    lost = []
    for client in client_list:
        ...
        try:
            last_date = datetime.fromisoformat(
                str(last_visit).replace("Z", "+00:00")
            ).date()
        except Exception:
            continue

        if three_months_ago <= last_date <= today:
            # Сохраняем существующую логику категорий, ограничив её 3 месяцами.
...
def report_dynamics(start, end):
    ...
    return (
        ...
        f"❌ Неявки: {number(current['noshow'])} → {pct_change(current['noshow'], previous['noshow'])}"
    )
...
def route_message(chat_id, text):
    ...
    if low in ("🔄 вернуть клиентов", "вернуть клиентов", "кого вернуть", "/вернуть"):
        return report_return(), menu_button()

    if low in ("💰 финансы", "финансы", "/финансы"):
        return report_finance(), menu_button()
    ...
    if "вернут" in low or "потерян" in low or "уходящ" in low:
        return report_return(), menu_button()

    if "финанс" in low or "выруч" in low:
        return report_finance(), menu_button()

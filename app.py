...
def report_dynamics():
    ...
    return (
        f"📈 ДИНАМИКА\n"
        f"🏪 {BRANCH_NAME}\n\n"
        f"7 дней vs предыдущие 7:\n"
        f"💰 Выручка: "
        f"{change(current_week['revenue'], previous_week['revenue'])}\n"
        f"👥 Клиенты: "
        f"{change(current_week['clients_count'], previous_week['clients_count'])}\n"
        f"🧾 Средний чек: "
        f"{change(current_week['avg_check'], previous_week['avg_check'])}\n\n"
        f"Месяц vs прошлый месяц:\n"
        f"💰 Выручка: "
        f"{change(current_month['revenue'], previous_month['revenue'])}\n"
        f"👥 Клиенты: "
        f"{change(current_month['clients_count'], previous_month['clients_count'])}\n"
        f"🧾 Средний чек: "
        f"{change(current_month['avg_check'], previous_month['avg_check'])}"
    )
...
def report_analysis():
    ...
    if current["noshow"] > previous["noshow"]:
        observations.append("🔴 Неявок больше, чем вчера.")
...
def report_clients():
    ...
    active = [
        ...
        if int(client.get("visits", 0) or 0) > 0
    ]
...
def route_message(text):
    ...
    if text in ("/динамика", "динамика", "📈 динамика"):
        return report_dynamics(), main_menu()
    ...
    if text in ("/финансы", "финансы", "💰 финансы"):
    ...
    if "финанс" in text or "выруч" in text:
        return report_finances(), main_menu()
